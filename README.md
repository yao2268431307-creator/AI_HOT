...
当前默认交付形态是**本机零订阅版**：Vinext/React 工作台、FastAPI、采集/评分 Worker、本地 PostgreSQL + pgvector、本地文件证据与本地 BGE-M3。默认不运行 Redis、MinIO/R2 或 OIDC，不启用任何计费连接器，外部数据预算被代码锁定为 0 元。原有云生产能力作为未来可选 profile 保留，不参与本机 V1 启动。

本机零订阅版的完整说明、命令和可信边界见 [本地零订阅运行手册](docs/LOCAL_ZERO_SUBSCRIPTION.md)。
...
当前冻结版本为 `product-metrics-2026-07-rc3.3`，冻结时间为 2026-07-17 15:20（Asia/Shanghai）。评分为 `score-0.7.0`，阈值为 `thresholds-2026-07-rc3`，迁移证明为 `001_init_rc3.1`。本次重新冻结绑定拒绝认证请求重定向的验收采集器、每轮 Redis/R2 探测、隔离删除角色、崩溃预算对账门槛和有界向量候选缓存；此前任何样本不得进入当前证据窗口。有效研判时长只累计认证 Analyst 的连续 `active` 心跳并要求至少 95% 覆盖，跨详情关闭/重开累计；客户端时长汇总被忽略，服务端心跳墙钟作为不能被 state 缩短的独立护栏。正式 Top5 必须逐项来自独立签名的完整排序账本，发现提前量使用版本化 append-only 阈值跨越事实。策略还绑定人工评估 schema、预登记 schema、四类隔离的 Ed25519 keyring 和验收 evaluator 的 SHA-256，任一变更都必须重新冻结并重新开始证据窗口。
...
从不可变镜像、托管数据库/R2、权限签批、灾备恢复到 72 小时连续采集的正式配置入口见 [生产配置与验收手册](docs/PRODUCTION_CONFIGURATION.md)。
需要组织管理员提供的最小输入见 [管理员交接单](docs/ADMIN_HANDOFF.md)。
...
docs/                        用例、实现边界与交付记录
docker-compose.yml           默认仅 PostgreSQL；Redis、MinIO 位于可选 full profile
```

## 本机零订阅运行

要求 Docker Desktop、Node.js 22+ 与 Python 3.12+。首次启动会安装本地多语模型运行库并尝试下载权重；不创建云账号，也不要求支付方式。若当前网络无法访问模型文件，启动不会失败，而是明确进入规则聚类降级状态，之后再次启动时重试下载。
...
只有未来需要恢复原云生产拓扑时才启动可选完整基础设施：

```powershell
docker compose --profile full up -d
```

Bluesky 在本机 profile 默认启用、在 production profile 默认关闭，仅用于实验候选发现。连接器会在四个官方公共 Jetstream 实例间故障转移，AppView 对 AT URI、CID 和作者 DID 做快照核验并写入原始 envelope；但在 durable update/delete supersession、撤回重评分和 72H soak 完成前，所有 Bluesky 项仍标记 `unverified_discovery`，不进入评分、独立信源计数或告警。该开关不代表数据权利或 72H soak 已通过。

未来 production profile 的计量连接器采用 fail-closed：除月度上限外，还必须通过 `CONNECTOR_COST_RMB_<CONNECTOR_ID>` 明确配置合约折算的人民币/请求成本；本机 profile 完全禁止这类连接器，并把预算固定为 0。

以下 Redis 告警 Worker 仅属于未来 production profile。本机 profile 由进程内 Dispatcher 直接消费 PostgreSQL Outbox，并只生成站内告警：
...
默认套件会跳过 18 项需要 PostgreSQL、Redis 或 S3 兼容对象存储的真实基础设施用例。启动 `docker compose` 并显式配置 `POSTGRES_INTEGRATION_DSN`、`POSTGRES_INTEGRATION_ADMIN_DSN`、`POSTGRES_INTEGRATION_DELETION_DSN`、`REDIS_INTEGRATION_URL` 与 `S3_INTEGRATION_*` 后，可验证实际迁移/RLS/trigger、隔离删除角色、并发去重、Outbox→Redis、consumer group、安全水位裁剪和 MinIO 对象操作。协调重启脚本还要求用 `DOCKER_INTEGRATION_CONTEXT` 指定经校验的本地 Docker context，并 fail-closed 限定 compose 的 loopback 端口 `5432/6379/9000`，不会接触远端服务：
...
正式模式会 fail closed，只有以下条件同时成立才有资格判定：PostgreSQL、受限 `radar_app` 角色、强制 RLS、精确匹配表/函数/schema/事件/启用状态的关键审计 triggers、只读迁移标记 `001_init_rc3.1`、短时 EdDSA/RS256 JWT 与服务端成员关系、稳定 `RADAR_INSTANCE_ID`、数据库/应用时钟偏差不超过 5 秒、Redis ping 与目标 R2 bucket 读写实探、五个运行组件的新鲜心跳、DR 证明、实际运行 API/Web 摘要与发布清单精确一致、冻结 keyring 中分离的 scheduler/reviewer/baseline/ledger 公钥，以及所有必需连接器获得显式数据权利批准。InMemory、录制 demo、空 keyring 和 `pending/blocked` 权利状态均不能通过正式 72H/7D。
...
- 本机 profile 的月度数据预算固定为 0 元；未来 production profile 才支持付费预算及 75%/90%/100% 分级降频。

## 生产前仍需完成

本机零订阅代码链路、真实公开源采集、本地备份恢复和零费用监控已经可运行；72 小时连续采集、120 个持续活跃信源、标注集 F1/聚类准确率以及 500 信源/50 万观测容量仍需以完整时长和真实样本验收。云端正式 Beta 的授权、镜像、灾备、多人身份和更大容量属于延后的 production 范围，详见 [实现状态](docs/IMPLEMENTATION_STATUS.md)。
