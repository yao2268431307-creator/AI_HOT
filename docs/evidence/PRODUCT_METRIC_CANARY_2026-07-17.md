...
> 历史证据说明：本文件记录的是 `rc2.1` 时代的一次本地 canary，只证明当时的 HTTP 契约与零样本保护。它已被 `product-metrics-2026-07-rc2.7` 的签名、运行时证明、完整排序/阈值跨越账本与人工预登记规则取代，不得作为当前正式 72H/7D 验收证据；当前策略必须重新开始完整证据窗口。
