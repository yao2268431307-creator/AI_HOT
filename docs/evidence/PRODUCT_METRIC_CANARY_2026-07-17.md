...
> 历史证据说明：本文件记录的是 `rc2.1` 时代的一次本地 canary，只证明当时的 HTTP 契约与零样本保护。它已被 `product-metrics-2026-07-rc2.12` 的签名、运行时证明、完整排序/阈值跨越账本、来源可信级别围栏与人工预登记规则取代，不得作为当前正式 72H/7D 验收证据；当前策略必须从 2026-07-17 08:38（Asia/Shanghai）重新开始完整证据窗口。
