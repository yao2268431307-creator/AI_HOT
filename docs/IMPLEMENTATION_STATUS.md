...
- RSS、Hacker News、GitHub、Hugging Face、arXiv、OpenAlex、YouTube 连接器适配器，以及 disabled-by-default 的实验 Bluesky 候选发现适配器；限流重试、URL 规范化、指纹去重、DNS/重定向 SSRF 拦截和真实原始证据归档。Bluesky 的 AppView 精确 URI/CID/DID 快照核验会写入原始 envelope，但在 durable update/delete supersession、撤回重评分和 72H soak 完成前，所有项仍是 `unverified_discovery`：不计分、不计独立/有效信源且不能满足告警证据门槛。AppView 服务级失败会使整轮 degraded 且不推进 checkpoint。跨来源搜索结果按 item 分片归档，避免因共享 batch 对象阻断选择性物理删除。
...
- rc2 产品 KPI 的冻结策略为 `product-metrics-2026-07-rc2.12`，因 Bluesky 收紧为 discovery-only、权利策略摘要上限变为可执行约束、并发 provenance 升级回归补齐，把迁移证明推进至 rc2.9，并改变绑定的验收监控摘要，于 2026-07-17 08:38（Asia/Shanghai）重新冻结；此前样本全部排除，证据窗口从该时点重启。系统在事件进入每个可研判 epoch 时原子记录 QueueEligibility，反馈必须引用当前有效 eligibility key，旧 epoch 不能污染首次分诊；强告警接受率使用已送达告警作分母和持久化反馈作判断源；错误强告警要求两个不同 Actor 复核。系统故障导致的重复告警只能由 Owner 基于两条真实投递建立不可变 MetricIncident，排除事实绑定 incident digest，不接受自由文本归因。
...
- 168 项默认 Python 自动化用例、14 项显式基础设施用例、Ruff、生产构建和 2 项 SSR/静态产品契约用例。
...
- X、Bilibili 的生产连接，以及 Bluesky 实验适配器向正式连接器的晋级；它们受授权、配额、协议稳定性或实时流部署约束。
...
- 目标生产环境的 PostgreSQL/RLS/Redis/R2 集成、跨节点中断、网络分区、进程硬终止和大规模重放演练。本地 Docker 已实际应用 `001_init_rc2.9`，并通过 RLS/trigger/只读迁移标记、并发信源去重、同 ID 并发 provenance 升级、并发 Outbox publisher、XADD/PG 标记失败窗口、生产消费者 ACK/DLQ、空 Stream 重建、Redis stale-writer 拒绝、MinIO put/read/delete 和协调重启持久性验证；这不替代目标环境验收，也不把 at-least-once 上调为跨系统 exactly-once。
...
```text
Python deterministic/default: 168 passed, 14 infrastructure tests skipped
Python with explicit local infrastructure: 182 passed, 7 upstream deprecation warnings
Python Ruff: passed
Web: ESLint passed
...
Bluesky live boundary: one low-level Jetstream message was observed, but later bounded adapter probes hit endpoint reset/handshake timeout; experimental acceptance remains blocked
```
...
Bluesky 的协议、来源可信级别、确定性用例和不稳定实时端点边界见 [Bluesky Jetstream 实验验证](evidence/BLUESKY_JETSTREAM_2026-07-17.md)。
